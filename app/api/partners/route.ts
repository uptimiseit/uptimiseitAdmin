...
export async function POST(req: Request) {
  try {
    const body = await req.json();
    const { fullName, workEmail, companyName, country, partnerType, message } = body;

    // Validation matching your UI mandatory fields
    if (!fullName || !workEmail || !country || !partnerType) {
      return NextResponse.json(
        { error: 'Required fields missing: Full Name, Work Email, Country, and Partner Type are mandatory.' }, 
        { status: 400, headers: corsHeaders }
      );
    }
    ...
    return NextResponse.json(
      { message: 'Partner protocol initiated.', id: result[0].id }, 
      { status: 201, headers: corsHeaders }
    );
    ...
    return NextResponse.json(
      { error: 'Failed to log partner intake.' }, 
      { status: 500, headers: corsHeaders }
    );
  }
}