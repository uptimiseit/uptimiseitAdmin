...
import Link from "next/link";
import { Edit3, Trash2, ExternalLink, Plus, Layers } from "lucide-react";
  ...
  return (
    <div className="p-8 space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-black tracking-tighter uppercase">Deployment <span className="text-blue-600">Ledger.</span></h1>
        <Link href="/admin/case-studies/create" className="bg-slate-950 text-white px-6 py-3 rounded-2xl font-bold flex items-center gap-2">
        ...
        <table className="w-full text-left">
          <thead className="bg-slate-50/50 border-b border-slate-50">
            <tr>
              ...
              <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Tag / Tech</th>
            ...
            {items.map((item: any) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="px-8 py-6">
                  <p className="font-bold text-slate-900 uppercase text-sm">{item.title}</p>
                  <p className="text-xs text-slate-400">{item.slug}</p>
                </td>
                <td className="px-8 py-6">
                  <span className="text-[10px] font-black text-blue-600 uppercase bg-blue-50 px-2 py-1 rounded">{item.tag}</span>
                  <p className="text-[10px] text-slate-400 mt-1 font-mono">{item.technology}</p>
                </td>
                <td className="px-8 py-6 text-right space-x-2">
                  <Link href={`/admin/case-studies/${item.id}`} className="inline-block p-2 text-slate-400 hover:text-blue-600"><Edit3 size={18}/></Link>
