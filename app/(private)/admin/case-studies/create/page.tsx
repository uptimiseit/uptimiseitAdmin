...
//     if (!seoData.pageTitle) setSeoData(prev => ({ ...prev, pageTitle: val }));
//   };
...
//       const studyRes = await createCaseStudy(payload);

//       if (studyRes.success && studyRes.id) {
//         await upsertSeoData("CASE_STUDY", studyRes.id, seoData);
...
//               <Briefcase className="text-indigo-600" size={24} /> New Case Study
//             </h1>
//           </div>
//         </div>
...
//           {isSaving ? "Saving..." : "Publish Case Study"}
//         </button>
//       </div>
...
//               <input type="text" required value={title} onChange={handleTitleChange} placeholder="Scaling a Fintech App..." className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100 font-bold" />
//             </div>

//             <div className="grid grid-cols-2 gap-4">
//               <div className="space-y-1.5">
//                 <label className="text-sm font-bold text-slate-800">URL Slug *</label>
//                 <input type="text" required value={slug} onChange={(e) => setSlug(e.target.value)} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100 font-mono text-sm" />
//               </div>
//               <div className="space-y-1.5">
...
//               <textarea rows={2} value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="2-sentence hook..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100 resize-none text-sm"></textarea>
//             </div>
//           </div>
...
//                 <input type="text" value={clientInfo.duration} onChange={(e) => setClientInfo({...clientInfo, duration: e.target.value})} placeholder="e.g. 6 Months" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-indigo-100" />
//               </div>
//             </div>
//           </div>
...
//                     <input type="text" value={step.title} onChange={(e) => updateArray(setProcess, process, index, "title", e.target.value)} placeholder="Step Name (e.g. Discovery & UI Design)" className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded outline-none font-bold" />
//                     <textarea rows={2} value={step.desc} onChange={(e) => updateArray(setProcess, process, index, "desc", e.target.value)} placeholder="Describe what happened in this phase..." className="w-full px-3 py-2 text-sm bg-white border border-slate-200 rounded outline-none resize-none"></textarea>
//                   </div>
...
export default function CreateCaseStudyPage() {
  return (
    <main className="min-h-screen bg-[#FDFDFF] pt-10">
      <CaseStudyForm />
    </main>
  );
}